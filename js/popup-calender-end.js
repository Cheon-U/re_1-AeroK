...
flatpickr(".startDate", {
    ...
    onChange: function (selectedDates, dateStr, instance) {
        ...
        if (selectedDates.length === 1) {
            const arrived = instance.formatDate(selectedDates[0], "Y-m-d");
            showDate.innerHTML = `도착일: ${arrived}`;

            // 부모창에 날짜 표시
            if (window.opener && !window.opener.closed) {
                const display = window.opener.document.querySelector('.popup-cal-arrived-display');
                if (display) {
                    // 날짜 표현 바꾸기
                    ...
                    const arrivedDate = `${year} / ${month} / ${day}`;

                    display.textContent = arrivedDate;
                }
            }
        } else {
            showDate.innerHTML = "선택된 날짜 없음";
...
console.log("display:", window.opener?.document.querySelector('.popup-cal-arrived-display'));
console.log(window.opener.document.querySelector('.popup-cal-arrived-display'));
